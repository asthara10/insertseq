# nf-core/insertseq: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/insertseq/usage](https://nf-co.re/insertseq/usage)
...
```console
nextflow run nf-core/insertseq --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/insertseq
```
...
First, go to the [nf-core/insertseq releases page](https://github.com/nf-core/insertseq/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
