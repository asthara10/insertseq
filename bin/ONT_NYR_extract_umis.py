...
def detect_read_strand(read_5p_seq, read_3p_seq, upstream_context_fwd):
    result_5 = edlib.align(upstream_context_fwd, read_5p_seq, mode="HW", k=5)
    result_3 = edlib.align(upstream_context_fwd, read_3p_seq, mode="HW", k=5)
    strand = "-"
    if result_5["editDistance"] < result_3["editDistance"]:
        strand = "+"
...
def extract_umis(
    ...
    for fasta_filename in input_files:
            ...
            with pysam.FastxFile(fasta_filename) as fh:
                for entry in fh:
                    ...
                    strand = detect_read_strand(
                        read_5p_seq, read_3p_seq, upstream_context_fwd
                    )
                    ...
                    if (strand == '+' and not result_5p_fwd_seq) or (strand == '-' and not result_3p_rev_seq):
                        continue
        ...
        if n_read:
            ...
            logging.info(
                "{}% of reads contained both (only one!) UMIs with max {} mismatches".format(
...
def main(argv=sys.argv[1:]):
    ...
    pattern_fwd = (
        "NNNYRNNNYRNNNYRNNN",
        ["N", "Y", "R"],
        [("N", "A"), ("N", "C"), ("N", "T"), ("N", "G"), ("Y", "C"), ("Y", "T"), ("R", "A"), ("R", "G")],
        True,
    )
    pattern_rev = (
        "NNNYRNNNYRNNNYRNNN",
